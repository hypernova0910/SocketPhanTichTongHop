...
namespace SocketServer.KMeanCluster
{
    class KMeanCluster2
    {
        public static List<Vertex> Cluster(List<Vertex> vertices, int numClusters)
        {
            double[][] rawData = new double[vertices.Count][];
            for(int i = 0; i < vertices.Count; i++)
            {
                rawData[i] = new double[] { vertices[i].X, vertices[i].Y };
            }
            ...
            //        else
            //        {
            //            result.Add(new Vertex(rawData[i][0], rawData[i][1], 0));
            //            break;
            //        }
            //    }
            //}
            ...
            for (int k = 0; k < numClusters; k++)
            {
                ...
                for (int i = 0; i < clustering.Length; i++)
                {
                    ...
                    {
                        count++;
                        Console.WriteLine(rawData[i][0]);
                        Console.WriteLine(rawData[i][1]);
                        sumX += rawData[i][0];
                        sumY += rawData[i][1];
                    }
                }
                double meanX = count != 0 ? sumX / count : 0;
                double meanY = count != 0 ? sumY / count : 0;
                result.Add(new Vertex(meanX, meanY, 0));
            }
            return result;
        }
