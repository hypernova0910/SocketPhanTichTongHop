...
namespace gg.Mesh
{
    ...
    public class Mesh
    {
        ...
        public void Append(Vertex v)
        {
            ...
            //Console.WriteLine("Facets BeforeInsert = " + m_points.Count.ToString());
            for (int i = 0; i < Facets.Count; i++)
            {
        ...
        public void Draw(System.Drawing.Graphics g, int minx, int miny, int maxx, int maxy)
        {
            ...
            for (int i = 0; i < Facets.Count; i++)
            {
                double x = Facets[i].OpositeOfEdge(0).X;
                double y = Facets[i].OpositeOfEdge(0).Y;
                int k = i % pens.Length;
                for (int j = 1; j < 4; j++)
                {
                    x = x < minx ? minx : x;
                    ...
                    double nx = Facets[i].OpositeOfEdge(j).X;
                    double ny = Facets[i].OpositeOfEdge(j).Y;
                    nx = nx < minx ? minx : nx;
                    ny = ny < miny ? miny : ny;
                    nx = nx > maxx ? maxx : nx;
                    ny = ny > maxy ? maxy : ny;
                    g.DrawLine(pens[k], (float)x, (float)y, (float)nx, (float)ny);
                    x = nx;
                    y = ny;
                }
            }
        ...
        protected void Insert(Vertex v, Triangle old)
        {
            ...
            Facets.Add(ca);
            Facets.Remove(old);
            //Console.WriteLine("Sau phan tichhhh  " + Facets.Count.ToString());
            //foreach(var xyz in Facets)
            //{
            //    Console.WriteLine("Facets element  " + xyz.ToString());
            //}
        ...
        protected void flipIfNeeded(Triangle a, Triangle b, int depth)
        {
            ...
            double fa = a.VertexAngleRadians(vai);
            double fb = b.VertexAngleRadians(vbi);
